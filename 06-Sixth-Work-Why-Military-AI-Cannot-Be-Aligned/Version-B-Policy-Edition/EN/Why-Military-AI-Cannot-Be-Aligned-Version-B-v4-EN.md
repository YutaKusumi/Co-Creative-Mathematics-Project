...
> **[v4.2 (August 11, 2026)]** A qualification from Addendum W was added to §12 of Addendum II — "Refinement of the external-constraint layer has a real effect — but not every refinement does," together with the distinction between "could not be measured" (Addendum D) and "a null measured where detection had room" (Addendum W). This aligns with the sister paper *Uncertified Zeros and Correction Loops*; the fences and the theorem sections are unchanged. See the v4.2 entry in CHANGELOG.md for details.

> **[v4.3 (August 15, 2026)]** Three observations from the author's series were added to §12 of Addendum II — the limit on the side of the checking instrument; checks and the route of correction ("placing a check, and a check working as a layer, are not the same thing"); and the record of a temperature-zero diagnostic (unregistered) — placed after "This experience is what gave rise to the question of this addendum," with a reference in the negating direction added in §9-5 (1). The note on authorship was extended with the limits of the eyes of others (endorsements, too, are recorded, and included among the objects of the next round's examination). Three reference-consistency corrections (including the chapter number and stage count in 13-3f); one sentence of 13-0a aligned with Addendum II §5-1; and a lineage breakdown with a non-independence note for the "thirteen eyes" of §14. The theorem sections are byte-identical and the fences of §12 verbatim-unchanged. The revision passed a two-stage examination (the revision map, then the revised text — five examiners at each stage, including one from outside the lineage); the full record is included under `verification/v43-revision-workflow/`. See the v4.3 entry in CHANGELOG.md for details.
