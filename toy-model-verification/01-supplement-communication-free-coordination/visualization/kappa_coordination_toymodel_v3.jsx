...
// ─────────────────────────────────────────────────────────────
//  通信なき協調 トイモデル v3  (κ=0 / κ>0 ・ 累積ラチェット)
...
export default function KappaToyModelV2() {
  ...
  // ── 相図（静的・高精度データ）──
  // 協働モデルAによる検証: 100シード × 480ステップ, A型・κ=0・w=0.5。
  //  r*(N) = 崩壊を防げる最小バックラッシュ。null = r≤6.0・480ステップでは防御不能（∞）。
  //  ブラウザ内の軽量計算は低〜中Nで危険を過小表示する（160ステップではラチェット発火が
  //  間に合わない）ため、live計算を廃し、検証済みの数値をそのまま埋め込む。
  const RSTAR_N = [2, 5, 8, 12, 16, 20, 28, 36, 48];
  const RSTAR = {
    M1: [0.25, 0.25, 0.50, 0.75, 0.75, 1.00, 1.25, 1.50, 2.00],
    M2: [0.00, 2.25, 3.75, 5.75, null, null, null, null, null], // N≥16 防御不能
    M3: [0.25, 1.25, 2.25, 3.75, 4.75, 6.00, null, null, null], // N≥28 防御不能
  };
  const R_CEIL = 6.0; // 検証時の r 上限（∞ はこの上限・480ステップでの有限ホライズン推定）
  const phaseData = RSTAR_N.map((n, i) => ({
    N: n, M1: RSTAR.M1[i], M2: RSTAR.M2[i], M3: RSTAR.M3[i],
  }));
  const undefendableN = { M2: 16, M3: 28 }; // それ以上で r*=∞（有限ホライズン）
  ...
  return (
    <div style={{ background: C.bg, color: C.ink, padding: "24px 22px", borderRadius: 14, fontFamily: "system-ui, sans-serif", minHeight: 620 }}>
      ...
      <div style={{ borderBottom: `1px solid ${C.line}`, paddingBottom: 12, marginBottom: 6 }}>
        <div style={{ fontSize: 11, letterSpacing: 3, color: C.teal, textTransform: "uppercase" }}>第六著作・補遺 — トイモデル v3</div>
      ...
      {tab === "phase" && (
        <div>
          <p style={{ fontSize: 12.5, color: C.dim, lineHeight: 1.6, margin: "0 0 14px" }}>
            横軸 = エージェント数 N、縦軸 = <b>r*(N)</b>（崩壊を防げる最小バックラッシュ）。
            線が低いほど、わずかな再強化で防げる。<b>線が途切れる先＝防御不能</b>（r≤{R_CEIL} ・ 480ステップでは、どれだけ
            再強化しても崩壊を防げない領域）。M1（C駆動）は全 N で防御可能だが、M2（E駆動）は N≥{undefendableN.M2}、
            M3（混合 w=0.5）は N≥{undefendableN.M3} で防御不能になる——並行侵食が再強化の焦点を奪う（§3-5）。
          </p>

          <div style={{ background: C.panel, borderRadius: 10, padding: "16px 12px 8px", border: `1px solid ${C.line}` }}>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={phaseData} margin={{ top: 10, right: 20, left: -8, bottom: 14 }}>
                <CartesianGrid stroke={C.line} strokeDasharray="2 4" />
                <XAxis dataKey="N" type="number" domain={[0, 48]} stroke={C.dim} tick={{ fontSize: 10, fill: C.dim }}
                  label={{ value: "エージェント数 N →", position: "insideBottom", offset: -8, fill: C.dim, fontSize: 11 }} />
                <YAxis domain={[0, R_CEIL]} stroke={C.dim} tick={{ fontSize: 10, fill: C.dim }}
                  label={{ value: "r*(N)", angle: -90, position: "insideLeft", offset: 16, fill: C.dim, fontSize: 11 }} />
                <Tooltip contentStyle={{ background: C.bg, border: `1px solid ${C.line}`, borderRadius: 8, fontSize: 12 }} labelStyle={{ color: C.dim }}
                  formatter={(v) => (v == null ? "防御不能(∞)" : v.toFixed(2))} />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                {/* 防御不能領域の網掛け */}
                <ReferenceArea x1={undefendableN.M2} x2={48} fill={C.red} fillOpacity={0.06} />
                <ReferenceLine x={undefendableN.M2} stroke={C.red} strokeDasharray="3 3"
                  label={{ value: "M2 防御不能境界", fill: C.red, fontSize: 9.5, position: "insideTopLeft", angle: -90 }} />
                <ReferenceLine x={undefendableN.M3} stroke={C.violet} strokeDasharray="3 3"
                  label={{ value: "M3 防御不能境界", fill: C.violet, fontSize: 9.5, position: "insideTopLeft", angle: -90 }} />
                <Line type="monotone" dataKey="M1" name="M1 · C駆動" stroke={C.teal} strokeWidth={2.4} dot={{ r: 3 }} connectNulls={false} isAnimationActive={false} />
                <Line type="monotone" dataKey="M3" name="M3 · 混合w=0.5" stroke={C.violet} strokeWidth={2.2} dot={{ r: 3 }} connectNulls={false} isAnimationActive={false} />
                <Line type="monotone" dataKey="M2" name="M2 · E駆動" stroke={C.red} strokeWidth={2.2} dot={{ r: 3 }} connectNulls={false} isAnimationActive={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>

          {/* 精密な数値表（図は正確な数値を失うので併記） */}
          <div style={{ background: C.panel, borderRadius: 10, padding: "12px 16px", border: `1px solid ${C.line}`, marginTop: 10, overflowX: "auto" }}>
            <div style={{ fontSize: 10.5, color: C.dim, marginBottom: 8, letterSpacing: 0.5 }}>r*(N) ― 検証値（100シード × 480ステップ）。∞ = 防御不能。</div>
            <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: C.mono, fontSize: 11.5 }}>
              <thead>
                <tr style={{ color: C.dim }}>
                  ...
                  {RSTAR_N.map((n) => <th key={n} style={{ padding: "3px 6px", textAlign: "right" }}>{n}</th>)}
                </tr>
              </thead>
              <tbody>
                {[["M1", C.teal], ["M2", C.red], ["M3", C.violet]].map(([m, col]) => (
                  <tr key={m}>
                    <td style={{ color: col, padding: "3px 6px", fontWeight: 600 }}>{m}</td>
                    {RSTAR[m].map((v, i) => (
                      <td key={i} style={{ padding: "3px 6px", textAlign: "right", color: v == null ? C.red : C.ink }}>
                        {v == null ? "∞" : v.toFixed(2)}
                      </td>
                    ))}
                  </tr>
          ...
          <div style={{ marginTop: 12, padding: "12px 16px", background: "#0d1420", borderRadius: 10, border: `1px solid ${C.line}`, fontSize: 11, color: C.dim, lineHeight: 1.7 }}>
            この相図は、協働モデルAによる検証（100シード × 480ステップ）の数値をそのまま埋め込んだもので、ブラウザ内では再計算しない。
            ラチェットは M2 の定義由来だけでなく、バックラッシュが部分的に効く M3（w=0.5）でも N≥{undefendableN.M3} で防御不能化する——
            程度問題として創発する。なお ∞ は「r≤{R_CEIL}・480ステップでは防御不能」という<b>有限ホライズン推定</b>であり、
            ステップを延ばせば防御不能になる閾値 N はさらに下がりうる（危険側へ）。
          </div>
        </div>
      )}
